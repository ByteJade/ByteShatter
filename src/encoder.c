    ...
    switch (buf->type) {
        ...
        case CMP:{
            if (t0 == REG) {
                ...
                emit_address_decode(context, &buf->a, SC1R, buf->prefix);
                emit32(context, _construct_r_r_imm(LDRB_REG, SC1, SC1, 0));
                emit32(context, _construct_r_r_imm(SUB_IMM|S, XZR, SC1, buf->b.imm));
    ...
    switch (buf->type) {
        ...
        case SAR:{
            if (t0 == REG && t1 == IMM)
                ...
                emit32(context, _construct_r_r_r(sf|ASR_REG, r0, r0, r1));
        ...
        case POP:{
            if (t0 == REG) {
                ...
                if (prev_instruction == POP) {
                    ...
                    emit32(context, LDP | (x64_regs[r0]<<10) | (SC2R<<5) | (prev_register));
                    emit_add_signed(context, 31, SC2R, 16);
                } else {
                    prev_instruction = POP;
                    prev_register = x64_regs[r0];
                    emit32(context, SF|ADD_IMM | (31<<5) | SC2R);
                    emit32(context, MFT|LDR32_REG | (SC2R<<5)| x64_regs[r0]);
